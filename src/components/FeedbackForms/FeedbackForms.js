...
import { useInterval, useProjectCountry } from '../../utils/customHooks/customHooks'
import Button from '../Button'
import Dialog, { DialogBody, DialogFooter, DialogHead, DialogNotification } from '../Dialog'
...
export function FeedbackDialog(props) {
  ...
  const subjectOptions = useMemo(() => {
    return SUBJECT_OPTIONS.map(({ text, value }) => ({ text: getText(text), value }))
  }, [getText])
  ...
  return (
    <Dialog
      className={`FeedbackDialog ${smileySelection ? 'smiley-selected' : 'no-smiley-selected'}`}
      data-testid="FeedbackDialog"
    >
      <DialogNotification>
        {error && (
          <Toast message={getText('ui-unexpected-error')} type="error" autoDismiss={null} />
        )}
      </DialogNotification>
        ...
        {!successfullySubmitted ? (
          <div className="form">
            <div className="form-section">
              <p className="input-label">{getText('ui-feedback-what-do-you-think')}</p>
              <SmileySelector selection={smileySelection} setSelection={setSmileySelection} />
            </div>
      ...
      {!successfullySubmitted && (
        <DialogFooter>
          ...
          <ActionButton className="btn action-button" disabled={!canSubmit} onClick={handleSubmit}>
...
export function ReportIssueDialog(props) {
  ...
  const { submit, email, setEmail, message, setMessage, canSubmit, successfullySubmitted } =
    useFeedbackDialogAPI(
      {
...
export function CompactFeedbackForm(props) {
  ...
  return (
    <div className={`CompactFeedbackForm ${className}`}>
      ...
      {!successfullySubmitted ? (
        <>
          <h2 className="heading">{getText('ui-feedback-compact-dialog-heading')}</h2>
          <SmileySelector selection={smileySelection} setSelection={setSmileySelection} />
          <FormInput
          ...
          />
          <PrivacyStatement />
          <ActionButton className="action-button" disabled={!canSubmit} onClick={handleSubmit}>
            {getText('ui-general-submit')}
          </ActionButton>
        </>
      ) : (
...
export function useFeedbackDialogAPI(options = {}, optionalFields = {}) {
  ...
  const executeRecaptcha = useRecaptcha(undefined, { handleBadgeVisibility: false })
  ...
  const canSubmit = useMemo(() => {
    ...
    return validEmail && validSmiley && validNumber && validMessage && validSubject && !submitting
  }, [
    validEmail,
    validSmiley,
    validNumber,
  ...
  async function submit() {
    ...
    try {
      ...
      const result = await api.post('/contact/feedback', dataToSend, { withKeyToken: true })

...
function DownloadDesignPDFButton(props) {
  ...
  function handleFinished(err) {
    if (!err) {
      ...
      (async () => {
        try {
          const { recaptchaToken, recaptchaNumber, domain } =
            await executeRecaptcha(SAVE_PREDESIGN_ACTION);
          await api.post(
            `/predesign/${designCtx.hiddenId}/count/downloaded`,
            {
              recaptchaToken,
              recaptchaNumber,
            },
  ...
  async function loadAddresses() {
    ...
    try {
      const result = await api.get(
        `/frontline/${nameToUse}/addresses?get_all=true`
      );
      return result;
    } catch (err) {
  ...
  async function generateFloorThumb(materials, images) {
    ...
    if (materials[0]?.id === "PVC") {
      ...
      let piece2;
      try {
        const pieceBin = await axios.get(images[0], {
          responseType: "arraybuffer",
        });
        piece1 =
          "data:image/png;base64," +
          Buffer.from(pieceBin.data, "binary").toString("base64");
      } catch {
        console.log("PDF image ", images[0], " missing");
      }
      try {
        const pieceBin = await axios.get(images[1], {
          responseType: "arraybuffer",
        });
        piece2 =
          "data:image/png;base64," +
      ...
      try {
        const pieceBin = await axios.get(images[2], {
          responseType: "arraybuffer",
        });
        piece3 =
          "data:image/png;base64," +
  ...
  async function generateMixedThumbs(data) {
    ...
    if (
      ...
      data.wallB &&
      data.wallB.mixed &&
      !data.wallB.generatedPdfImage
    ) {
      thumbs.generatedPdfImageBWall = await generateWallThumb(
        data.wallB.pdfImages
      );
    }
    if (
      data &&
      data.wallC &&
      data.wallC.mixed &&
      !data.wallC.generatedPdfImage
    ) {
      thumbs.generatedPdfImageCWall = await generateWallThumb(
        data.wallC.pdfImages
      );
    }
    if (
      data &&
      data.wallD &&
      data.wallD.mixed &&
      !data.wallD.generatedPdfImage
    ) {
      thumbs.generatedPdfImageDWall = await generateWallThumb(
        data.wallD.pdfImages
      );
    }
    ...
    if (
      ...
      data.floor &&
      data.floor.mixed &&
      !data.floor.generatedPdfImage
    ) {
      thumbs.generatedPdfImageFloor = await generateFloorThumb(
        data.floor.materials,
        data.floor.pdfImages
      );
    }
  ...
  async function loadPdfData() {
    const designImagesObject =
      designCtx.designImages?.reduce((prev, curr) => {
        const result = { ...prev };
        result[curr.id] = curr;
        return result;
      }, {}) || {};
    ...
    if (designInformation.cop.image) {
      designInformation.cop.imageSize = await addImageSize(
        designInformation.cop.image
      );
    }
    if (designInformation.horCop.image) {
      designInformation.horCop.imageSize = await addImageSize(
        designInformation.horCop.image
      );
    }
    if (designInformation.hl.image) {
      designInformation.hl.imageSize = await addImageSize(
        designInformation.hl.image
      );
    }
    if (designInformation.lcs.image) {
      designInformation.lcs.imageSize = await addImageSize(
        designInformation.lcs.image
      );
    }
    if (designInformation.dop.image) {
      designInformation.dop.imageSize = await addImageSize(
        designInformation.dop.image
      );
    }
    if (designInformation.din.image) {
      designInformation.din.imageSize = await addImageSize(
        designInformation.din.image
      );
    }
    if (designInformation.eid.image) {
      designInformation.eid.imageSize = await addImageSize(
        designInformation.eid.image
      );
    }
    if (designInformation.fb.image) {
      designInformation.fb.imageSize = await addImageSize(
        designInformation.fb.image
      );
    }
    ...
    const data = {
      ...
      designUrl,
      tenderInfo:
        designCtx?.design?.tenderInfo && !designCtx.edited
          ? designCtx.design.tenderInfo
          : null,
    };

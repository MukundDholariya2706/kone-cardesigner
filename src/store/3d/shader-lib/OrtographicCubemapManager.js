...
import SimpleRawVertexShader from "./utility-shaders/simple-raw-3d-vert-glsl";
import WorldNormalShader from "./utility-shaders/draw-world-normals-frag.glsl";
import CubeHelperFragmentShader from "./utility-shaders/cube-texture-helper-frag-glsl";
...
export class OrtographicCubemapManager {
  constructor(
    ...
    this.landingStandardCamera = this.createStandardCamera(
      "Landing standard camera"
    );

    Object.values(this.cameras.landing).forEach((element) => {
      element.setTransformParameters(
        this.materialManager.boxProjectionData.landingSize.value,
        this.materialManager.boxProjectionData.landingPosition.value
      );
    });
    const debugScaleFactor = 0.75;
    this.debug.textureHelper.landing.scale.set(
      LANDING_CUBE_MAP_SIZE[0] * debugScaleFactor,
      LANDING_CUBE_MAP_SIZE[1] * debugScaleFactor,
      LANDING_CUBE_MAP_SIZE[2] * debugScaleFactor
    );
    this.debug.textureHelper.landing.position.set(
    ...
    this.debug.carMaterial.uniforms.cubemap.value =
      this.cameras.car.diffuseCamera.lastUpdatedTarget.texture;
    this.debug.landingMaterial.uniforms.cubemap.value =
      this.cameras.landing.diffuseCamera.lastUpdatedTarget.texture;
    ...
    Object.values(this.cameras.car.diffuseCamera.cameraHelpers).forEach(
      (element) => {
        scene.add(element);
      }
    );
    Object.values(this.cameras.landing.diffuseCamera.cameraHelpers).forEach(
      (element) => {
        scene.add(element);
      }
    );
  ...
  createCamera(name) {
    const camera = new OrthographicCubeCamera(
      0.1,
      1000,
      CUBE_CAMERA_RESOLUTION,
      this.cubemapOptions
    );
    camera.name = name;
    return camera;
  ...
  captureDefaultNormals() {
    ...
    this.materialManager.defaultNormalCubemap.value =
      this.defaultNormalCamera.texture;
  ...
  setBoxParameters() {
    ...
    /*
        ...
        */
    this.debug.textureHelper.car.scale.set(
      bpcemSize.x * debugScaleFactor,
      bpcemSize.y * debugScaleFactor,
      bpcemSize.z * debugScaleFactor
    );
  ...
  updateMaps() {
    ...
    this.materialManager.setOrthoEnvTextures(
      ENV_CAR,
      this.cameras.car.diffuseCamera.texture,
      this.cameras.car.ormCamera.texture,
      this.cameras.car.normalCamera.texture
    );
    this.materialManager.setOrthoEnvTextures(
      ENV_LANDING,
      this.cameras.landing.diffuseCamera.texture,
      this.cameras.landing.ormCamera.texture,
      this.cameras.landing.normalCamera.texture
    );
    this.materialManager.standardCarEnvTexture.value =
      this.carStandardCamera.renderTarget.texture;

    this.mirrorManager.setMirrorsQualityMode(hadRealtimeReflections);
    this.carStandardCamera.update(this.renderer, this.scene);
    this.debug.defaultMapMaterial.uniforms.cubemap.value =
      this.carStandardCamera.renderTarget.texture;

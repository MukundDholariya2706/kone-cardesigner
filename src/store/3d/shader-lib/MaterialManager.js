...
import {
  DEFAULT_LIGHT_MAP_INTENSITY,
  DEFAULT_ENV_MAP_INTENSITY,
  CUSTOM_ENV_MAP_BLUR_STEPS,
  QUALITY_3D_HIGH,
  QUALITY_3D_MEDIUM,
  QUALITY_3D_LOW,
  REFLECTION_BOUNCES_HIGH,
  REFLECTION_BOUNCES_MEDIUM,
  REFLECTION_BOUNCES_LOW,
} from "./3d-constants";
import {
...
import { CarDesignerMaterial } from "../legacy-interfaces";
import { CustomFragShader, CustomVertShader } from "./custom-other";
...
class MaterialManager {
  ...
  hasMaterial(id) {
    return (
      ...
      this.materials.find((item) => item.id === id) !== undefined
    );
  }
  ...
  createMaterial(materialDefinition) {
    ...
    if (specialOverride !== undefined) {
      ...
      if (materialDefinition.alphaMap) {
        specialMaterial.uniforms.alphaMap = {
          value: this.assetManager.getTexture(materialDefinition.alphaMap),
        };
      } else {
        specialMaterial.uniforms.alphaMap = {
          value: this.assetManager.getTexture(DEFAULT_WHITE_TEXTURE_ID),
        };
      }
      specialMaterial.uniforms.opacity = {
        value:
          materialDefinition.opacity !== undefined
            ? materialDefinition.opacity
            : 1,
      };
      specialMaterial.uniforms.uv2AlphaMap = {
        value: materialDefinition.useUv2WithAlphaMap
          ? materialDefinition.useUv2WithAlphaMap
          : true,
      };
      if (materialDefinition.emissiveMap) {
        specialMaterial.uniforms.emissiveMap = {
          value: this.assetManager.getTexture(materialDefinition.emissiveMap),
        };
      } else {
        specialMaterial.uniforms.emissiveMap = {
          value: this.assetManager.getTexture(DEFAULT_WHITE_TEXTURE_ID),
      ...
      if (materialDefinition.map) {
        specialMaterial.uniforms.colorMap = {
          value: this.assetManager.getTexture(materialDefinition.map),
        };
      } else {
        specialMaterial.uniforms.colorMap = {
          value: this.assetManager.getTexture(DEFAULT_WHITE_TEXTURE_ID),
      ...
      specialMaterial.uniforms.emissive = {
        value:
          materialDefinition.emissive !== undefined
            ? new THREE.Color(
                Number(materialDefinition.emissive)
              ).convertSRGBToLinear()
            : new THREE.Color(0, 0, 0),
      };
      specialMaterial.uniforms.emissiveIntensity = {
        value:
          materialDefinition.emissiveIntensity !== undefined
            ? materialDefinition.emissiveIntensity
            : 1,
      };
      specialMaterial.uniforms.bloomIntensity = {
        value:
          materialDefinition.bloomIntensity !== undefined
            ? materialDefinition.bloomIntensity
            : 0,
      };
      ...
      specialMaterial.uniforms.color = {
        value:
          materialDefinition.color !== undefined
            ? new THREE.Color(
                Number(materialDefinition.color)
              ).convertSRGBToLinear()
            : new THREE.Color(1, 1, 1),
      };
      specialMaterial.transparent = true; // not always the case, but shouldn't hurt
      if (materialDefinition.useTriplanarMapping) {
        specialMaterial.uniforms.useTriplanarMapping = {
          value: materialDefinition.useTriplanarMapping,
        };
        specialMaterial.uniforms.triplanarMappingScale = {
          value: materialDefinition.triplanarMappingScale,
        };
      }
    ...
    const materialParameters =
      this.createMaterialParameters(materialDefinition);
    ...
    if (useOldMat) {
      ...
      material = new CarDesignerMaterial(
        materialParameters,
        this,
        materialDefinition
      );
  ...
  populateUndefinedParameters(params, name) {
    ...
    for (const defaultPair of defaultMaps) {
      ...
      if (defaultPair.type === "normalMap" && hasBumpMap) {
        ...
        !params.hasOwnProperty(defaultPair.type) ||
        params[defaultPair.type] === undefined
      ) {
        let defaultOptions = {};
        ...
        const replacement = this.assetManager.getTexture(
          defaultPair.id,
          defaultOptions
        );
  ...
  createMaterialParameters(data) {
    ...
    for (const mapType of mapTypes) {
      if (data.hasOwnProperty(mapType)) {
        ...
        if (
          mapType === "envMap" &&
          [ENV_CAR, ENV_LANDING].indexOf(data[mapType]) !== -1
        ) {
    ...
    data.clippingPlanes !== undefined &&
      (params.clippingPlanes = data.clippingPlanes);
    data.stencilWrite !== undefined &&
      (params.stencilWrite = data.stencilWrite);
    ...
    !isNaN(data.refractionRatio) &&
      (params.refractionRatio = data.refractionRatio);
    ...
    !isNaN(data.envMapIntensity || DEFAULT_ENV_MAP_INTENSITY) &&
      (params.envMapIntensity =
        data.envMapIntensity || DEFAULT_ENV_MAP_INTENSITY);
    !isNaN(data.emissiveIntensity) &&
      (params.emissiveIntensity = data.emissiveIntensity);
    !isNaN(data.lightMapIntensity || DEFAULT_LIGHT_MAP_INTENSITY) &&
      (params.lightMapIntensity =
        data.lightMapIntensity || DEFAULT_LIGHT_MAP_INTENSITY);
    !isNaN(data.reflectivity) && (params.reflectivity = data.reflectivity);
    !isNaN(data.transmission) && (params.transmission = data.transmission);
    !isNaN(data.clearcoat) && (params.clearcoat = data.clearcoat);
    !isNaN(data.clearcoatRoughness) &&
      (params.clearcoatRoughness = data.clearcoatRoughness);
    ...
    for (const mapType of mapTypes) {
      if (materialDefinition[mapType] !== material.userData[mapType]) {
        if (materialDefinition[mapType]) {
          if (
            mapType === "envMap" &&
            [ENV_CAR, ENV_LANDING].indexOf(materialDefinition[mapType]) !== -1
          ) {
            //... set env later material[mapType] = this.getEnvTexture(materialDefinition.envMap)
          } else {
            material[mapType] = this.assetManager.getTexture(
              materialDefinition[mapType]
            );
          }
        } else {

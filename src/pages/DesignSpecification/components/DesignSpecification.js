...
const DesignSpecification = (props) => {
  ...
  const designUrl = getShareLink(
    designStore.hiddenId,
    siteCtx.isMarine,
    domainCountry
  );
  ...
  const customFinishesInUse = useMemo(() => {
    ...
    return getCustomFinishesInUse(
      designStore.design,
      productStore.customFinishes
    );
  ...
  const saveAllowed = useMemo(() => {
    ...
    if (
      customFinishesInUse &&
      customFinishesInUse.filter((x) => !x.shared && !x.finish.mixed).length > 0
    ) {
      return false;
    }
  ...
  const { designImages, setDesignImages } = designStore;
  ...
  const executeRecaptcha = useRecaptcha(DOWNLOAD_PAGE_ACTION, {
    visible: true,
  });
  ...
  useEffect(() => {
    return async () => {
      if (
        designStore?.design?.country === "ktoc" &&
        productStore?.productKTOCDestCountry
      ) {
  ...
  useEffect(() => {
    ...
    if (!designStore.design) {
      ...
      if (d) {
        ...
        setAnalyticsForEditPage({
          collectionId: "",
          countryCode: countryFromDomain,
          countryId: country,
          productName: getText(productStore.product.name),
          languageCountryId: countryCode,
          languageId: langCode,
          templateId: d.sapId,
        });
        return;
  ...
  useEffect(() => {
    if (!designStore.design) return;
    const designInfo = getDesignInformation({
      designStore,
      productStore,
      offeringStore,
      getText,
  ...
  const navBarChildren = (
    <>
      <Link
        className="navbar-item edit-design-button"
        to={`${props.match.url.replace("specification", "edit")}`}
      >
  ...
  return (
    <>
      ...
      <Layout
        ...
        navBarClassName={`navbar-lg pr-4 pl-4 design-specification${
          fromShareLink ? " hideBackButton" : ""
        }`}
          ...
          <div className="DesignSpecification content">
            ...
            <div className="topContainer">
              <h2 className="page-header">
                {getText("ui-general-design-specification")}
              </h2>
              <DesignImages
                ...
                productId={
                  designStore?.design?.product || designStore?.design?.productId
                }
              />
            </div>
            ...
            designImages.map(element => {
                ...
                <div className="textContent">
                  <DesignNameEditor
                    ...
                    isEditable={(() => {
                      return (
                        designStore.edited ||
                        designStore.design.ownedDesign ||
                        designStore.design.ktoc
                      );
                    })()}
                    executeRecaptcha={() =>
                      executeRecaptcha(SAVE_PREDESIGN_ACTION)
                    }
                  />
                  <p className="model">
                    {getText(productStore?.product?.name)}
                  </p>
                  <p className="desc">
                    {getText(productStore?.product?.description)}
                  </p>
                </div>
              </div>
              <ShareDesignArea
              ...
              {unsupportedComponents && unsupportedComponents.length > 0 && (
                <div
                  ref={unsupportedRef}
                  className="block unsupportedComponents"
                >
                  ...
                  <div className="unsupportedComponents__container">
                    {unsupportedComponents.map((item) => {
                      return (
                        <p key={item.itemType} className="unsupportedComponent">
              ...
              <div className={"contact-container"}>
                <div
                  className="contact"
                  onClick={(e) => setShowContactDialog(true)}
                >
